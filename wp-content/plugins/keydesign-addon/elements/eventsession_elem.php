...
if (!class_exists('KD_ELEM_EVENT_SESSION')) {
    ...
    class KD_ELEM_EVENT_SESSION extends KEYDESIGN_ADDON_CLASS {
        ...
        public function kd_eventsession_shrt($atts, $content = null) {
            ...
            // Declare empty vars
            $output = $speaker_image = $href = $link_target = $link_title = $es_unique_id = $expand_class = $content_image = $animation_delay = $wrapper_class = $speaker_img_array = '';
            ...
            $output = '<div class="'.trim($wrapper_class).'" id="'.$es_unique_id.'" '.(!empty($es_background_color) ? 'style="background-color: '.$es_background_color.';"' : '').' '.$animation_delay.'>
						    ...
						    $output .= '<div class="es-speaker-container">';
                                if ( '' != $es_image || '' != $ext_image ) {
                                    $output .= '<div class="es-speaker-img">' . $content_image . '</div>';
                                }
                                if ( '' != $es_speaker_name ) {
                                    $output .= '<div class="es-speaker-name">' . $es_speaker_name . '</div>';
                                }
                                if ( '' != $es_speaker_position ) {
                                    $output .= '<div class="es-speaker-position">' . $es_speaker_position . '</div>';
                                }
                            $output .= '</div>
