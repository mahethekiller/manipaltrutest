...
class PTBS_Importer {
     ...
     *
     * @param bool $fresh If true, wipes existing ptbs_test and ptbs_package posts before importing
    ...
    public static function run( $fresh = false ) {
        ...
        if ( $fresh ) {
            $wpdb->query( "DELETE pm FROM {$table_prefix}postmeta pm INNER JOIN {$table_prefix}posts p ON pm.post_id = p.ID WHERE p.post_type IN ('ptbs_test', 'ptbs_package')" );
            $wpdb->query( "DELETE tr FROM {$table_prefix}term_relationships tr INNER JOIN {$table_prefix}posts p ON tr.object_id = p.ID WHERE p.post_type IN ('ptbs_test', 'ptbs_package')" );
            $wpdb->query( "DELETE FROM {$table_prefix}posts WHERE post_type IN ('ptbs_test', 'ptbs_package')" );
        }
        ...
        return array(
            ...
            'message'            => sprintf(
                'Successfully synced %d Tests, %d Packages (%d test links), %d Subcategories, and %d Conditions!',
                $test_count, $pkg_count, $total_links, count( $subcategory_map ), count( $condition_map )
            )
        );
    }
}
